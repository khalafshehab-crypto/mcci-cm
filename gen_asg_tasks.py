...
import { 
  ...
  Clock, Edit2, Trash2, X, ListTodo, User
} from "lucide-react";
...
const CATEGORIES = ["مراجعة تقرير", "اعتماد ميزانية", "متابعة قرار", "توجيه إداري", "أخرى"];
const STATUSES = ["قيد التنفيذ", "متأخرة", "مكتملة"];
const PRIORITIES = ["عالية", "متوسطة", "عادية"];

export default function AssistantSecGenTasks() {
  const { data: tasks } = useFirestoreCollection<ASGTask>("assistant_sec_gen_tasks", []);
  
  const [searchQuery, setSearchQuery] = useState("");
  const [statusFilter, setStatusFilter] = useState("all");
  
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingTask, setEditingTask] = useState<ASGTask | null>(null);
  
  const [formData, setFormData] = useState<Partial<ASGTask>>({
    title: "",
    description: "",
    category: "متابعة قرار",
    assignedTo: "",
    dueDate: "",
    status: "قيد التنفيذ",
    priority: "متوسطة"
  ...
  const handleOpenModal = (t?: ASGTask) => {
    if (t) {
      setEditingTask(t);
      setFormData(t);
    } else {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingTask?.id) {
        await updateDoc(doc(db, "assistant_sec_gen_tasks", editingTask.id), formData);
      } else {
        await addDoc(collection(db, "assistant_sec_gen_tasks"), formData);
      }
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("هل أنت متأكد من حذف هذه المهمة؟")) {
      try {
        await deleteDoc(doc(db, "assistant_sec_gen_tasks", id));
      } catch (error) {
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6" dir="rtl">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div>
          <h1 className="text-2xl font-black text-gray-900 flex items-center gap-2">
            ...
            المهام الإدارية لمساعد الأمين العام
          </h1>
          <p className="text-gray-500 text-sm mt-1">متابعة وتوجيه المهام الإدارية والقرارات</p>
        </div>
        <button
      ...
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row gap-4 items-center">
        <div className="relative flex-1 w-full">
          ...
          <input
            type="text"
            placeholder="بحث في المهام والمكلفين..."
            value={searchQuery}
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <form onSubmit={handleSave} className="p-6 space-y-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-700 mb-1.5">التفاصيل / التوجيه</label>
                  <textarea
                    required
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="اسم الموظف أو الإدارة"
                      className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-brand focus:bg-white transition-all text-sm font-bold"
                    />
                  </div>
                  <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-gray-700 mb-1.5">تصنيف المهمة</label>
                    <select
