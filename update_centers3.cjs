...
// Fix grid layout. Row 3 has md:col-span-2 for MOWAZAF, and md:col-span-1 for QAA.
// Let's change MOWAZAF to md:col-span-1.
code = code.replace(/<div className="space-y-1 md:col-span-2">\s*<label className="text-\[11px\] font-black text-gray-500 block">الموظف المختص \*/g, 
  '<div className="space-y-1 md:col-span-1">\n                            <label className="text-[11px] font-black text-gray-500 block">الموظف المختص *');
...
code = code.replace(
  /<\/select>\s*<\/div>\s*<\/div>\s*<div className="space-y-1 md:col-span-3">/g,
  `</select>
                          </div>
...
// Fix grid for series. It was grid-cols-3. We'll change it to grid-cols-4 or just let it wrap. Let's let it wrap by not changing the parent grid.
...
code = code.replace(
  /<\/select>\s*<\/div>\s*<\/div>\s*<\/div>\s*\{!isConfirmingSeries \? \(/g,
  `</select>
                          </div>
