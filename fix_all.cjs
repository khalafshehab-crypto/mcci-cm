...
// The proxy fetch block
const proxyBlock = `  const response = await fetch("/api/google-proxy", {
    method: "POST",
...
// Let's just regex replace everything from `const response = await fetch("https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&supportsAllDrives=true"`
// down to `return response.json();`

code = code.replace(/const response = await fetch\("https:\/\/www\.googleapis\.com\/upload\/drive\/v3\/files\?uploadType=multipart&supportsAllDrives=true"[\s\S]*?return response\.json\(\);\n/g, proxyBlock + '\n');
