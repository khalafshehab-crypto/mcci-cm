...
code = code.replace(
  /\/\/ Check if already requested\s*const res = await fetch\('\/api\/join-requests'\);\s*const allReqs = await res\.json\(\);\s*const requestExists = allReqs\.find\(\(req: any\) => req\.email\?\.trim\(\)\.toLowerCase\(\) === emailLower\);/,
