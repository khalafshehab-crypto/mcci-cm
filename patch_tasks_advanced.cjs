...
pages.forEach(page => {
  ...
  if (content.includes('const handleSaveAction = async (e: FormEvent) => {') && !content.includes('action: "تحديث إنجاز"')) {
    ...
    try {
      const colName = (currentTask as any)._sourceCol || "${page.includes('Centers') ? 'centers_tasks' : page.includes('Affiliates') ? 'affiliates_tasks' : page.includes('Assistant') ? 'assistant_sec_gen_tasks' : 'tasks'}";
  ...
  if (content.includes('const handleImmediateEscalate = async (task: TaskItem) => {') && !content.includes('action: "تصعيد"')) {
    ...
    if (nextLevel !== task.escalationLevel) {
      ...
      try {
        const colName = (task as any)._sourceCol || "${page.includes('Centers') ? 'centers_tasks' : page.includes('Affiliates') ? 'affiliates_tasks' : page.includes('Assistant') ? 'assistant_sec_gen_tasks' : 'tasks'}";
...
`;
  ...
  // 9. Update the task cards and table view to display history log preview ? The history log will just appear in the achievement notes if combinedNotes works.
