...
const newDirectFunc = `async function fetchGoogleAPI(endpoint: string, options: RequestInit = {}): Promise<any> {
...
if (code.match(regex)) {
  code = code.replace(regex, newDirectFunc);
  fs.writeFileSync('src/lib/googleApi.ts', code);
  console.log("Replaced fetchGoogleAPI to direct fetch successfully!");
} else {
