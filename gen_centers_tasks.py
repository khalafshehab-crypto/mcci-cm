...
import { 
  ...
  Clock, Edit2, Trash2, X, ListTodo, User, Building2
} from "lucide-react";
...
export default function CentersTasks() {
  const { data: tasks } = useFirestoreCollection<CenterTask>("centers_tasks", []);
  ...
  const [editingTask, setEditingTask] = useState<CenterTask | null>(null);
  
  const [formData, setFormData] = useState<Partial<CenterTask>>({
    title: "",
    description: "",
  ...
  const filteredTasks = useMemo(() => {
    return tasks.filter(t => {
      ...
      const matchesCenter = centerFilter === "all" || t.centerName === centerFilter;
      const matchesStatus = statusFilter === "all" || t.status === statusFilter;
      return matchesSearch && matchesCenter && matchesStatus;
    }).sort((a, b) => new Date(a.dueDate).getTime() - new Date(b.dueDate).getTime());
  }, [tasks, searchQuery, centerFilter, statusFilter]);
  ...
  const handleOpenModal = (t?: CenterTask) => {
    if (t) {
      setEditingTask(t);
      setFormData(t);
    } else {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingTask?.id) {
        await updateDoc(doc(db, "centers_tasks", editingTask.id), formData);
      } else {
        await addDoc(collection(db, "centers_tasks"), formData);
      }
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("هل أنت متأكد من حذف هذه المهمة؟")) {
      try {
        await deleteDoc(doc(db, "centers_tasks", id));
      } catch (error) {
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div>
          <h1 className="text-2xl font-black text-gray-900 flex items-center gap-2">
            ...
            مهام المراكز التابعة
          </h1>
          <p className="text-gray-500 text-sm mt-1">متابعة تنفيذ مهام ومشاريع المراكز التابعة</p>
        </div>
        <button
            ...
            >
              ...
              <form onSubmit={handleSave} className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-gray-700 mb-1.5">المركز المعني</label>
                    <select
