...
export default function AffiliatesEvents() {
  const { data: events } = useFirestoreCollection<AffiliateEvent>("affiliates_events", []);
  ...
  const [formData, setFormData] = useState<Partial<AffiliateEvent>>({
    title: "",
    date: "",
    time: "",
    location: "",
  ...
  const filteredEvents = useMemo(() => {
    return events.filter(evt => {
      ...
      const matchesSector = sectorFilter === "all" || evt.targetSector === sectorFilter;
  ...
  const handleOpenModal = (evt?: AffiliateEvent) => {
    if (evt) {
      setEditingEvent(evt);
      setFormData(evt);
    } else {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingEvent?.id) {
        await updateDoc(doc(db, "affiliates_events", editingEvent.id), formData);
      } else {
        await addDoc(collection(db, "affiliates_events"), formData);
      }
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("هل أنت متأكد من حذف هذه الفعالية؟")) {
      try {
        await deleteDoc(doc(db, "affiliates_events", id));
      } catch (error) {
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6" dir="rtl">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">إدارة فعاليات ولقاءات المنتسبين وقطاعات الأعمال</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-gray-500 font-bold">فعاليات قادمة</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredEvents.map(evt => (
          ...
          >
            ...
            <div className="flex items-center gap-1.5 mb-4 text-brand bg-brand/5 w-fit px-2 py-0.5 rounded text-xs font-bold">
              ...
              <span>قطاع: {evt.targetSector}</span>
            </div>
            ...
            <div className="mt-auto space-y-2 pt-3 border-t border-gray-50">
              ...
              <div className="flex items-center justify-between text-xs text-gray-600">
                ...
                <span className="font-bold px-1.5 py-0.5 bg-gray-100 rounded shrink-0">{evt.expectedAttendees} شخص</span>
              </div>
            </div>
          </motion.div>
        ))}
        {filteredEvents.length === 0 && (
          <div className="col-span-full py-12 text-center bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200">
            ...
            <p className="text-gray-500 font-medium">لا توجد فعاليات مسجلة للمنتسبين أو مطابقة لبحثك</p>
          </div>
        )}
      </div>
            ...
            >
              ...
              <form onSubmit={handleSave} className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-gray-700 mb-1.5">القطاع المستهدف</label>
                    <select
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-700 mb-1.5">العدد المتوقع</label>
                    <input
                      required
                      type="number"
                      min="1"
