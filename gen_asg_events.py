...
import { 
  Calendar, MapPin, Search, Plus, Filter, AlertCircle, CheckCircle2, 
  Clock, Edit2, Trash2, X, FileText, Briefcase
} from "lucide-react";
...
const CATEGORIES = ["اجتماع إداري داخلي", "لقاء خارجي", "استقبال وفد", "ورشة عمل", "أخرى"];
const STATUSES = ["مجدول", "تم الانتهاء", "مؤجل", "ملغي"];

export default function AssistantSecGenEvents() {
  const { data: events } = useFirestoreCollection<ASGEvent>("assistant_sec_gen_events", []);
  ...
  const [editingEvent, setEditingEvent] = useState<ASGEvent | null>(null);
  
  const [formData, setFormData] = useState<Partial<ASGEvent>>({
    title: "",
    date: "",
    time: "",
    location: "",
    category: "اجتماع إداري داخلي",
    status: "مجدول",
  ...
  const stats = useMemo(() => {
    return {
      ...
      completed: events.filter(e => e.status === "تم الانتهاء").length,
      requiresPrep: events.filter(e => e.requiresPreparation && e.status === "مجدول").length
    };
  }, [events]);

  const handleOpenModal = (evt?: ASGEvent) => {
    if (evt) {
      setEditingEvent(evt);
      setFormData(evt);
    } else {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingEvent?.id) {
        await updateDoc(doc(db, "assistant_sec_gen_events", editingEvent.id), formData);
      } else {
        await addDoc(collection(db, "assistant_sec_gen_events"), formData);
      }
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("هل أنت متأكد من حذف هذه الفعالية؟")) {
      try {
        await deleteDoc(doc(db, "assistant_sec_gen_events", id));
      } catch (error) {
  ...
  const getStatusColor = (status: string) => {
    switch(status) {
      ...
      case "تم الانتهاء": return "bg-emerald-100 text-emerald-800 border-emerald-200";
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6" dir="rtl">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div>
          <h1 className="text-2xl font-black text-gray-900 flex items-center gap-2">
            ...
            فعاليات واجتماعات مساعد الأمين العام
          </h1>
          <p className="text-gray-500 text-sm mt-1">إدارة وجدولة المواعيد الخاصة بمكتب مساعد الأمين العام</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-gray-500 font-bold">مجدولة وقادمة</p>
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-gray-500 font-bold">تم الانتهاء</p>
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-gray-500 font-bold">تتطلب تجهيز</p>
            <p className="text-xl font-black text-gray-900">{stats.requiresPrep}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredEvents.map(evt => (
          ...
          >
            ...
            <p className="text-xs text-gray-500 font-medium mb-4">{evt.category}</p>
            ...
            <div className="mt-auto space-y-2">
              ...
              {evt.requiresPreparation && (
                <div className="mt-3 p-2 bg-amber-50 rounded-lg border border-amber-100 flex items-start gap-2">
                  <AlertCircle className="w-3.5 h-3.5 text-amber-600 shrink-0 mt-0.5" />
                  <p className="text-[10px] text-amber-800 leading-relaxed font-medium">
                    <span className="font-bold block mb-0.5">يتطلب تجهيز مسبق:</span>
                    {evt.preparationNotes || "لا توجد ملاحظات"}
                  </p>
                </div>
              )}
              {evt.importedFrom && (
                <div className="mt-2">
                  <span className="inline-block px-1.5 py-0.5 rounded text-[10px] font-black bg-purple-100 text-purple-800 border border-purple-200">
                    مستورد من: {evt.importedFrom}
                  </span>
                </div>
              )}
            </div>
        ...
        {filteredEvents.length === 0 && (
          <div className="col-span-full py-12 text-center bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200">
            ...
            <p className="text-gray-500 font-medium">لا توجد فعاليات مسجلة أو مطابقة لبحثك</p>
          </div>
        )}
      </div>
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <form onSubmit={handleSave} className="p-6 space-y-4">
                <div>
                  <label className="block text-xs font-bold text-gray-700 mb-1.5">عنوان الفعالية / الاجتماع</label>
                  <input
                    required
                    type="text"
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-700 mb-1.5">المكان</label>
                    <input
                      type="text"
                ...
                <div className="pt-2 border-t border-gray-100">
                  <label className="flex items-center gap-2 cursor-pointer mb-3">
                    ...
                    />
                    <span className="text-sm font-bold text-gray-700">يتطلب تجهيز مسبق</span>
                  </label>
                  {formData.requiresPreparation && (
                    <div>
                      <label className="block text-xs font-bold text-gray-700 mb-1.5">تفاصيل التجهيز المطلوب</label>
                      <textarea
