...
export default function Centers() {
  ...
  const { data: dbEvents } = useFirestoreCollection<any>("centers_events", []);
  const { data: dbMembers } = useFirestoreCollection<any>("members", []);
  const { data: dbRecs } = useFirestoreCollection<any>("recommendations", []);
  const { data: dbTasks } = useFirestoreCollection<any>("centers_tasks", []);
  ...
  const handleSubmitReferral = async () => {
    ...
    try {
      if (selectedAlarm.type === "task") {
        ...
        const docRef = doc(db, "centers_tasks", taskId);
        ...
        if (matchedEvent) {
          ...
          const docRef = doc(db, "centers_events", String(matchedEvent.id));
          await updateDoc(docRef, {
  ...
  const handleMarkUrgent = async (alarm: Alarm, e: React.MouseEvent) => {
    ...
    try {
      if (alarm.type === "event") {
        ...
        await updateDoc(doc(db, "centers_events", rawId), {
          status: isUrgentNow ? "عاجل" : "دوري",
          isUrgentOverride: isUrgentNow
        });
      } else if (alarm.type === "task") {
        await updateDoc(doc(db, "centers_tasks", alarm.id), {
