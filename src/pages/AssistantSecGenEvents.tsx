...
وتفضلوا بقبول وافر التحية والتقدير،،،`;
  ...
  const handleInsertSeries = async () => {
    ...
    const stats = await syncEventsToCalendar(newEventsList, dbEmployees, "assistant_sec_gen_events", allMembers);
      if (stats.failed > 0) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (editingEvent) {
      ...
      const stats = await syncEventsToCalendar([updatedEvent], dbEmployees, "assistant_sec_gen_events", allMembers);
      if (stats.failed > 0) {
      ...
      const stats = await syncEventsToCalendar([newEvent], dbEmployees, "assistant_sec_gen_events", allMembers);
      if (stats.failed > 0) {
