...
interface EventItem {
  ...
  status: "تجهيز الفعاليات" | "تأكيد الموعد مع رئيس المركز" | "إرسال الدعوات" | "تأكيد الحضور" | "محضر الاجتماع" | "التوصيات" | "منتهية" | string;
...
export default function CentersEvents() {
  ...
  const { data: events, addDocument: addFirebaseEvent, updateDocument: updateFirebaseEvent, deleteDocument: deleteFirebaseEvent } = useFirestoreCollection<EventItem>("centers_events", []);
...
const [viewMode, setViewMode] = useState<"grid" | "table" | "calendar">("grid");
  ...
  const getCalculatedNextStep = (evt: EventItem) => {
    if (!evt.committeeConfirmed) {
      return "تأكيد الموعد مع رئيس المركز";
    }
  ...
  const getStepIndex = (nextStep: string) => {
    switch (nextStep) {
      case "تأكيد الموعد مع رئيس المركز": return 0;
  ...
  const updateEventWorkflow = (eventId: number, updates: Partial<EventItem>) => {
    ...
    if (targetEvent && !canUserEditCommittee(targetEvent.committeeName)) {
      alert("عذراً، لا تملك الصلاحية لتعديل فعاليات هذه اللجنة. يمكنك فقط إدارة فعاليات المراكز المكلف بها.");
      return;
    }
...
const [singleClassification, setSingleClassification] = useState("");
  ...
  const handleOpenEdit = (evt: EventItem) => {
    if (!canUserEditCommittee(evt.committeeName)) {
      alert("عذراً، لا تملك الصلاحية لتعديل هذه الفعالية. يمكنك فقط تعديل فعاليات المراكز المكلف بها.");
      return;
    }
    setEditingEvent(evt);
  ...
  const handleOpenDelete = (evt: EventItem) => {
    if (!canUserEditCommittee(evt.committeeName)) {
      alert("عذراً، لا تملك الصلاحية لحذف هذه الفعالية. يمكنك فقط تعديل فعاليات المراكز المكلف بها.");
      return;
    }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (editingEvent) {
      ...
      try {
        await updateDoc(doc(db, "centers_events", String(editingEvent.id)), updatedEvent);
      ...
      try {
        await setDoc(doc(db, "centers_events", String(newEvent.id)), newEvent);
      } catch (err) {
        console.error(err);
      }
  ...
  const handleDelete = async () => {
    if (deletingEvent) {
      try {
        await deleteDoc(doc(db, "centers_events", String(deletingEvent.id)));
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case "تأكيد الموعد مع رئيس المركز": return "text-indigo-600 bg-indigo-100 ring-indigo-200";
          ...
          <div className="bg-white border border-gray-150 rounded-xl sm:rounded-2xl p-4 shadow-sm flex flex-col xl:flex-row xl:items-center justify-between gap-2.5 sm:gap-3 md:gap-4 font-sans">
            <div className="flex flex-wrap items-center gap-2 text-xs font-black text-gray-700">
              ...
              >
                ...
                <span>الرئيسية (لوحة المراكز)</span>
              </button>
...
<React.Fragment>
                  ...
                  >
                    ...
                    <span>
                      {rawCommittees.find((c) => c.id === selectedCommIdForCards)?.name || "المركز المحدد"}
                    </span>
                  </button>
                </React.Fragment>
              )}
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="bg-slate-50/70 p-4 rounded-xl sm:rounded-2xl border border-slate-100 space-y-2.5 text-xs">
                        <div className="flex items-center justify-between">
                          <span className="text-gray-400 font-bold text-[10px]">رئيس المركز:</span>
                        ...
                        <td className="whitespace-nowrap px-4 py-3.5 whitespace-nowrap font-black text-gray-900 group/row" title="انقر لتشغيل منصة التحضير">
                          <div className="flex flex-col text-right truncate">
                            ...
                            <div className="text-[9.5px] text-gray-500 font-bold">
                              رئيس المركز: {allMembers.find(m => m.committeeId === evt.committeeId && m.role === "رئيس")?.name || "غير محدد"} - الموظف: {evt.employees[0] || "غير محدد"}
                            </div>
                          </div>
                        </td>
                              ...
                              <div className={`flex flex-col md:flex-row gap-3 sm:gap-4 md:gap-6 relative ${!canUserEditCommittee(evt.committeeName) ? "opacity-80 pointer-events-none grayscale-[10%]" : ""}`}>
                                ...
                                <div className="w-full md:w-1/3 flex flex-col gap-2.5 bg-white p-4 rounded-xl border border-gray-200 shadow-sm shrink-0">
                                  ...
                                  {(() => {
                                    ...
                                    const stepList = [
                                      { title: "تأكيد الموعد مع رئيس المركز", desc: "التنسيق وتأكيد موعد وقاعة الفعالية", done: !!evt.committeeConfirmed },
                                  ...
                                  {(() => {
                                    ...
                                    switch (curTab) {
                                          ...
                                          <div className="space-y-4">
                                            <div className="flex items-center justify-between pb-2 border-b border-gray-100">
                                              <h3 className="text-xs font-black text-slate-800 flex items-center gap-1.5">
                                                ...
                                                تنسيق وتأكيد موعد وقاعة الاجتماع مع رئيس المركز
                                              </h3>
                                              <span className="text-[9px] text-gray-500 font-bold leading-none">مرحلة 1 من 8</span>
                                            </div>
                                            
                                            <p className="text-[10px] text-gray-550 leading-relaxed font-bold">
                                              يتعين على الأخصائي الاتصال تلفونياً أو عبر البريد الإلكتروني برئيس المركز (أو نائب الرئيس حال انشغال الرئيس) للتوافق على موعد الفعالية، واعتماده رسمياً قبل إتمام الدعوة للأعضاء.
                                            </p>
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5 text-right font-sans">
                                              <div className="p-3 bg-slate-50/70 rounded-lg border border-gray-150">
                                                <span className="block text-[8px] text-brand font-black mb-1">بيانات رئيس المركز</span>
                                                {president ? (
                                                  <div>
                                              ...
                                              <div className="p-3 bg-slate-50/70 rounded-lg border border-gray-150 text-right">
                                                <span className="block text-[8px] text-[#4ea0b0] font-black mb-1 font-bold">نائب رئيس المركز</span>
                                                {vicePres ? (
                                                  <div>
                                            ...
                                            <div className="pt-3 border-t border-gray-100 flex items-center justify-between">
                                              <label className="flex items-center gap-2.5 cursor-pointer">
                                                ...
                                                <span className="text-[10px] text-slate-900 font-extrabold select-none">
                                                  تم التواصل واعتماد الموعد وتفاصيل القاعة لتأكيد الجدول مع رئيس المركز
                                                </span>
                                              </label>
                                        ...
                                        return (
                                          <div className="flex flex-col gap-2.5 sm:gap-3 md:gap-4 text-right">
                                            ...
                                            <div className="text-[9.5px] text-gray-500 leading-relaxed font-bold bg-slate-50 p-2.5 rounded-lg border border-gray-200 order-3">
                                              💡 <span className="text-slate-900">تعليمات الحضور:</span> يشترط حضور نصف الأعضاء المعينين على الأقل للجنة، شريطة وجود رئيس المركز أو نائب الرئيس لتسجيل اعتماد نصاب الفعالية القانوني.
                                             </div>
                                      ...
                                      case 4: { // Step 5: Agenda Builder
										...
										const commSpecialist = eventComm?.specialist || "أخصائي المركز";
													...
													<div className="grid grid-cols-1 md:grid-cols-12 gap-2 text-right">
														...
														<div className="md:col-span-4 flex flex-col gap-1">
															...
															>
																...
																{commSpecialist && (
																	<optgroup label="أخصائي المركز المسؤول">
																		<option value={commSpecialist}>{commSpecialist} (أخصائي المركز)</option>
																	</optgroup>
																)}
                                                  ...
                                                  <div key={item.id || `item-${idx}`} className="p-3.5 border border-gray-200 rounded-lg bg-slate-50 space-y-3 shadow-inner">
                                                    ...
                                                    <div className="grid grid-cols-1 md:grid-cols-12 gap-3 text-right pt-2.5 border-t border-dashed border-gray-200 font-sans items-end">
                                                      <div className="md:col-span-5 flex flex-col gap-1">
                                                        ...
                                                        <select
                                                          value={(item.assignee === "الأخصائي" && evt.employees?.[0]) ? `${evt.employees[0]} (أخصائي المركز)` : (item.assignee === "الأخصائي" ? "أخصائي المركز" : (item.assignee || ""))}
                                                        ...
                                                        >
                                                          ...
                                                          <option value={evt.employees?.[0] ? `${evt.employees[0]} (أخصائي المركز)` : "أخصائي المركز"}>
                                                            {evt.employees?.[0] ? `${evt.employees[0]} (أخصائي المركز)` : "أخصائي المركز"}
                                                          </option>
...
<div key="filter-popover-1784704070964-9" className="contents">
                          {/* Row 1 */}
                          <div className="space-y-1">
                            <label className="text-[11px] font-black text-gray-500 block">المركز *</label>
                            <select
                            ...
                            >
                              <option value={0} disabled>اختر المركز</option>
