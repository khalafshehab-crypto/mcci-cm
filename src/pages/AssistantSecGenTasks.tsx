...
export default function AssistantSecGenTasks() {
  ...
  const handleAddTask = async (e: FormEvent) => {
    ...
    try {
      await addDoc(collection(db, "assistant_sec_gen_tasks"), newTask);
      setIsAddOpen(false);
    } catch (e) {
      console.error(e);
    }
  ...
  const openActionModal = (task: TaskItem) => {
    ...
    try {
      const colName = (currentTask as any)._sourceCol || "assistant_sec_gen_tasks";
  ...
  // Quick Action triggers
    ...
    if (nextLevel !== task.escalationLevel) {
      ...
      try {
        const colName = (task as any)._sourceCol || "assistant_sec_gen_tasks";
