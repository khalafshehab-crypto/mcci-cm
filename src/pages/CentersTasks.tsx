...
export default function CentersTasks() {
  ...
  useEffect(() => {
    const q = query(collection(db, "centers_tasks"));
  ...
  const handleAddTask = async (e: FormEvent) => {
    ...
    try {
      await addDoc(collection(db, "centers_tasks"), newTask);
      setIsAddOpen(false);
    } catch (e) {
      console.error(e);
    }
  ...
  const handleEditTask = async (e: FormEvent) => {
    ...
    try {
      await updateDoc(doc(db, "centers_tasks", currentTask.id), {
        title,
        description,
        sourceType,
        sourceDetails,
  ...
  const openActionModal = (task: TaskItem) => {
    ...
    try {
      const colName = (currentTask as any)._sourceCol || "centers_tasks";
  ...
  // Quick Action triggers
    ...
    if (nextLevel !== task.escalationLevel) {
      ...
      try {
        const colName = (task as any)._sourceCol || "centers_tasks";
  ...
  const handleConfirmDeleteTask = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await deleteDoc(doc(db, "centers_tasks", taskToDeleteId));
    } catch (err) {
      console.error(err);
    }
