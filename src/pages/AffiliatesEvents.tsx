...
وتفضلوا بقبول وافر التحية والتقدير،،،`;
  ...
  const handleInsertSeries = async () => {
    ...
    const stats = await syncEventsToCalendar(newEventsList, dbEmployees, "affiliates_events");
      if (stats.failed > 0) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (editingEvent) {
      ...
      const stats = await syncEventsToCalendar([updatedEvent], dbEmployees, "affiliates_events");
      if (stats.failed > 0) {
      ...
      const stats = await syncEventsToCalendar([newEvent], dbEmployees, "affiliates_events");
      if (stats.failed > 0) {
