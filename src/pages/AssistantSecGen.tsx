...
export default function AssistantSecGen() {
  ...
  const { data: dbEventsCommittees } = useFirestoreCollection<any>("events", []);
  const { data: dbEventsCenters } = useFirestoreCollection<any>("centers_events", []);
  const { data: dbEventsAffiliates } = useFirestoreCollection<any>("affiliates_events", []);
  const { data: dbEventsSecGen } = useFirestoreCollection<any>("assistant_sec_gen_events", []);

  // Compute aggregated events
  const dbEvents = React.useMemo(() => {
    const all = [
      ...(dbEventsCommittees || []).map((e: any) => ({ ...e, sourceDept: "إدارة اللجان" })),
      ...(dbEventsCenters || []).map((e: any) => ({ ...e, sourceDept: "إدارة المراكز", committeeName: e.centerName || "مركز" })),
      ...(dbEventsAffiliates || []).map((e: any) => ({ ...e, sourceDept: "إدارة المنتسبين", committeeName: "منتسب" })),
      ...(dbEventsSecGen || []).map((e: any) => ({ ...e, sourceDept: "مساعد الأمين العام", committeeName: e.location || "مساعد الأمين العام" }))
    ];
    return all;
  }, [dbEventsCommittees, dbEventsCenters, dbEventsAffiliates, dbEventsSecGen]);
      ...
      if (Array.isArray(evts)) {
        ...
        activeEvts.forEach((evt: any) => {
          ...
          list.push({
            ...
            dept: evt.sourceDept || "إدارة الفعاليات",
