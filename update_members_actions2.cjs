...
const regexBtn = /\{\/\* Add member button \*\/\}\s*<button\s*type="button"\s*onClick=\{\(\) => setIsAddSelectionOpen\(true\)\}\s*className="h-10 px-4 bg-blue-600 hover:bg-blue-700 text-white font-black text-xs rounded-xl flex items-center justify-center gap-1\.5 shadow-sm hover:shadow transition-all duration-200 cursor-pointer"\s*>\s*<Plus className="w-4\.5 h-4\.5 stroke-\[2\.5\]" \/>\s*<span>إضافة عضو<\/span>\s*<\/button>/;

const newBtn = `{/* Merged Add/Import/Export Dropdown */}
...
if (code.match(regexBtn)) {
  ...
  console.log("Replaced dropdown successfully!");
} else {
  console.log("Dropdown regex not found!");
}

// Remove `isAddSelectionOpen` completely
const regexSelection = /\{\/\* ADD SELECTION MODAL \*\/\}\s*<AnimatePresence>\s*\{isAddSelectionOpen && \([\s\S]*?\}\s*<\/AnimatePresence>/;
if (code.match(regexSelection)) {
  code = code.replace(regexSelection, '');
  console.log("Removed Add Selection Modal!");
}

const regexState = /const \[isAddSelectionOpen, setIsAddSelectionOpen\] = useState\(false\);/;
if (code.match(regexState)) {
  code = code.replace(regexState, '');
  console.log("Removed state isAddSelectionOpen!");
}
