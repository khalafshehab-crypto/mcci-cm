...
const openCloudRegex = /\{cloudSelectOpen === t\.id && \([\s\S]*?\{t\.committeeUrls\.map\(\(cu: any\) => \([\s\S]*?<a key=\{cu\.committeeId\} href=\{cu\.folderUrl \|\| cu\.documentUrl\} target="_blank" rel="noopener noreferrer"/;
const openCloudReplace = `{cloudSelectOpen === t.id && (
                                <div className="absolute bottom-full mb-1 left-0 w-full bg-white border border-gray-200 rounded-lg shadow-lg z-10 p-1 flex flex-col gap-1">
                                    {t.committeeUrls.map((cu: any) => (
                                        <a key={cu.committeeId} href={cu.documentUrl || cu.folderUrl} target="_blank" rel="noopener noreferrer"`;
code = code.replace(openCloudRegex, openCloudReplace);

const directCloudReplace = /<a\s*href=\{t\.committeeUrls\?\.\[0\]\?\.folderUrl \|\| t\.cloudUrl\}/;
code = code.replace(directCloudReplace, '<a\n                          href={t.committeeUrls?.[0]?.documentUrl || t.cloudUrl}');
