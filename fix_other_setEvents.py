...
for filename in ['src/pages/Events.tsx', 'src/pages/Recommendations.tsx']:
    ...
    # Some old ones had this:
    c1 = r'''setEvents(events.map(ev => ev.id === editingEvent.id ? updatedRec : ev));'''
    c2 = r'''updateFirebaseEvent(String(editingEvent.id), updatedRec);'''
    content = content.replace(c1, c2)

    c3 = r'''setEvents([newRec, ...events]);'''
    c4 = r'''addFirebaseEvent(newRec);'''
    content = content.replace(c3, c4)
