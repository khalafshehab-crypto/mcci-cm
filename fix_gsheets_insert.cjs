...
// The one in AttachmentInput
const badInsert = `  const [googleSheetUrl, setGoogleSheetUrl] = useState("");
...
// Replace it back
content = content.replace(badInsert, '  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {');

// Now we need to insert it inside CommitteesMembers.
// Let's find: const [isImportOpen, setIsImportOpen] = useState(false);
const correctInsert = `
  // Import Excel States
...
content = content.replace(/  \/\/ Import Excel States\n  const \[isImportOpen/, correctInsert);
