...
# Fix commName in step 3
content = content.replace('{commName}', '{aiGenCommittees.map(id => committees.find(c => String(c.id) === id)?.name || id).join("، ")}')

# Fix finalDocumentText scope in saveAIGeneratedLetter
old_save = '''        if (finalType === "مستندات" || workspaceService === "circular") {
          try {
...
new_save = '''        let finalDocumentText = aiGenGeneratedText;
        if (workspaceService === "circular") {
...
# Clean up duplicate let finalDocumentText declaration which I mistakenly put earlier.
content = content.replace('''        let finalDocumentText = aiGenGeneratedText;
        if (workspaceService === "circular") {
            const circularBody = aiGenGeneratedText.split("عرض التعميم:")[1]?.trim() || aiGenGeneratedText.split("نص توجيهي مقترح لإرساله للجان:")[1]?.trim() || aiGenGeneratedText;
            finalDocumentText = `تعميم داخلي\\nاللجنة: ${committeeName}\\n\\nإلى: جميع أعضاء اللجان الموقرين\\nمن: إدارة اللجان\\nوارد من: ${circularIncomingFrom || "—"}\\nالتاريخ والرقم: ${circularNumberDate || "—"}\\nالموضوع: ${circularSubject || "—"}\\n\\n${circularBody}\\n\\nشاكرين ومقدرين تعاونكم،،،`;
        }
        
        let finalDocumentText = aiGenGeneratedText;''', '''        let finalDocumentText = aiGenGeneratedText;''')
