const fs = require('fs');
const filepath = 'src/pages/CommitteesLibrary.tsx';
...
// In case it was defined differently:
code = code.replace(
...
// We need to carefully replace the {smartLetterMode === "create" && ( ... )} block 
// with fine-grained checks.
code = code.replace(
  /\{smartLetterMode === "create" && \(/g,
  '{(smartLetterMode === "create_new" || smartLetterMode === "create_reply") && ('
);

// We need to hide the reply block if it's "create_new"
code = code.replace(
...
// Change the "حفظ القالب" button logic
code = code.replace(
  /\{smartLetterMode === "create" && \(/g,
  '{(smartLetterMode === "create_new" || smartLetterMode === "create_reply") && ('
);
